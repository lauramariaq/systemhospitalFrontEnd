      ...
      ></v-text-field>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">Agregar Habitacion</v-btn>
          </template>
          <v-card>
            <v-card-title>
    ...
    lazy-validation
         v-model="valid">
            ...
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="createRoom">Guardar</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-toolbar>
  ...
  export default {
   ...
   axios.put('Rooms/DeleteRoom/'+item.id).then(() =>{
    ...
    editRoom(roomObj){
      ...
      this.type = roomObj.type;
      this.price = roomObj.price;
      this.editedIndex = 1;
      this.dialog = true;
      }
